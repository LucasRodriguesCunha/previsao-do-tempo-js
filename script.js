...
async function buscarCidade(cidade) {
  const dados = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${cidade}&appid=${chaveAPI_openweathermap}&lang=pt_br&units=metric`
...
function mostrarDadosNaTela(dados) {
  ...
  document.querySelector(".cidade").innerHTML =
    "> Cidade: " + `<strong>${dados.name}<strong>`;

  // Exibir temperatura
  document.querySelector(".temperatura").innerHTML =
    "> Temperatura atual de " +
    `<strong>${Math.floor(dados.main.temp)}<strong>` +
    "°C";
  ...
  // Ícone
  document.querySelector(".img-icone").src =
